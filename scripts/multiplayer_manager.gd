...
@export var maze_path: NodePath = NodePath("../GameRoot")
...
func _spawn_player_for_peer(peer_id: int) -> void:
	...
	rpc("client_spawn_player", peer_id, spawn_transform)

@rpc("authority", "reliable", "call_local")
func client_spawn_player(peer_id: int, spawn_transform: Transform3D) -> void:
	if player_scene == null or _players_root == null:
		return
...
func _get_spawn_transform(peer_id: int) -> Transform3D:
	if _maze and _maze.has_method("get_corner_cell_world"):
		...
		var pos: Vector3 = corner_pos
		if _maze.has_method("world_to_cell") and _maze.has_method("get_cell_center"):
			var cell: Vector2i = _maze.world_to_cell(corner_pos)
			pos = _maze.get_cell_center(cell)
			if _maze.has_method("get_random_cell_position"):
				var padding := _get_spawn_padding()
				pos = _maze.get_random_cell_position(cell, padding)
				var center: Vector3 = _maze.get_cell_center(cell)
				pos.x = clamp(pos.x, center.x - padding, center.x + padding)
				pos.z = clamp(pos.z, center.z - padding, center.z + padding)
		pos.y = spawn_height
